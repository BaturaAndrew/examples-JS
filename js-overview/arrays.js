...
function interestingBypass() {
  ...
  console.log('[Init]:');
  commentList.forEach( comment => console.log(comment.author) );
...
function funcBotGen() {
  function* botGenerator() {
    let answer = yield 'I`m ready';
  
    while(true) {
      switch (answer) {
        case '1': {
          answer = yield 'I`ve got 1';
          break;
        }
        case '2': {
          answer = yield 'I`ve got 2';
          break;
        }
        default: {
          answer = yield 'I don`t understand :(';
          break;
        }
      }
    }
